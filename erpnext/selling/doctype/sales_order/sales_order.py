...
def make_maintenance_schedule(source_name, target_doc=None):
	def update_item(source, target, source_parent):
		target.start_date = source_parent.contract_start_date
		target.end_date = source_parent.contract_end_date
	...
	if not maint_schedule:
		doclist = get_mapped_doc("Sales Order", source_name, {
			...
			"Sales Order Item": {
				...
				"postprocess": update_item,
				"condition": lambda doc: frappe.db.get_value("Item", doc.item_code, "is_stock_item") == 1
			}
		}, target_doc)
