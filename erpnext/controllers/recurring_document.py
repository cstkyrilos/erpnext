...
def create_recurring_documents():
	...
	manage_recurring_documents("Journal Entry")
	...
	for ref_document, recurring_id in recurring_documents:
		if not frappe.db.sql("""select name from `tab%s`
			...
			try:
				...
				new_doc = make_new_document(doctype, reference_doc, date_field, next_date)
...
def make_new_document(doctype, reference_doc, date_field, posting_date):
	...
	if doctype == "Journal Entry":
		for i, item in enumerate(new_document.accounts):
			for fieldname in ("page_break",):
				item.set(fieldname, reference_doc.accounts[i].get(fieldname))
	else:
		for i, item in enumerate(new_document.items):
			for fieldname in ("page_break","sales_order","so_detail",):
...
def notify_errors(doc, doctype, party, owner):
	...
	assign_task_to_owner(doc, doctype, "Recurring Document Failed", recipients)
